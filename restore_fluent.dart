...
void main() {
  ...
  for (final file in dir.listSync().whereType<File>()) {
    ...
    // add import
    if (!content.contains("import 'package:fluent_ui/fluent_ui.dart'")) {
      content = "import 'package:fluent_ui/fluent_ui.dart' as fluent;\n" + content;
    }
    ...
    if (mappings.containsKey(filename)) {
      ...
      content = content.replaceFirst('if (NaPlatformInfoService.isIOS) {', 'if (NaPlatformInfoService.isWindows) {\n      \$fluentReturn\n    } else if (NaPlatformInfoService.isIOS) {');
    }
    
    // Special case for naShowAsync in alert dialog
    if (filename == 'na-platform-alert-dialog.widget.dart') {
      content = content.replaceFirst('if (NaPlatformInfoService.isIOS) {', '''if (NaPlatformInfoService.isWindows) {
